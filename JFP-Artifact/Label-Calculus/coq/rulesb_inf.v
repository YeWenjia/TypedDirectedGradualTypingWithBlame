...
Fixpoint close_term_wrt_term_rec (n1 : nat) (x1 : var) (t1 : term) {struct t1} : term :=
  match t1 with
    ...
    | trm_cast t2 p b A1 B1 => trm_cast (close_term_wrt_term_rec n1 x1 t2) p b A1 B1
  end.
...
Fixpoint size_term (t1 : term) {struct t1} : nat :=
  match t1 with
    ...
    | trm_cast t2 p b A1 B1 => 1 + (size_term t2) + (size_typ A1) + (size_typ B1)
  end.
...
Inductive degree_term_wrt_term : nat -> term -> Prop :=
  ...
  | degree_wrt_term_trm_cast : forall n1 t1 p b A1 B1,
    degree_term_wrt_term n1 t1 ->
    degree_term_wrt_term n1 (trm_cast t1 p b A1 B1).

Scheme degree_term_wrt_term_ind' := Induction for degree_term_wrt_term Sort Prop.

Definition degree_term_wrt_term_mutind :=
  fun H1 H2 H3 H4 H5 H6 H7 H8  =>
  degree_term_wrt_term_ind' H1 H2 H3 H4 H5 H6 H7 H8 .
