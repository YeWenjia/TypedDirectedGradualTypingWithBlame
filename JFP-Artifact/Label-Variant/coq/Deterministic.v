Require Import LibTactics.
Require Import Metalib.Metatheory.
Require Import Logic. Import Decidable.
Require Import
        syntax_ott
        rules_inf
        Typing
        Infrastructure.

Require Import Strings.String.

Lemma sim_refl: forall A,
  sim A A.
Proof.
  intros.
  inductions A; eauto.
Qed.


Lemma TypedReduce_unique: forall v r1 r2 (A: typ) l b,
    value v -> (exists B, Typing nil v Inf B) -> TypedReduce v l b A r1 -> TypedReduce v l b A r2 -> r1 = r2.
Proof.
  introv Val H R1 R2.
  gen r2.
  lets R1': R1.
  induction R1; introv R2;
    try solve [inverts* R2].
  - inverts* R2.
    inverts H2.
  - inverts* R2.
    inverts* H9.
  - inverts* R2.
    inverts* H0. 
  - inverts* R2.
    inverts* H10.
Qed.



Lemma sfill_eq: forall E0 e0 E e1 r1 r2,
  simpl_fill E0 e0 = simpl_fill E e1 ->
  step e0 r1 ->
  step e1 r2 ->
  simpl_wf E ->
  simpl_wf E0 ->
  E = E0 /\ e0 = e1.
Proof.
  introv eq red1 red2 wf1 wf2. gen E e0 e1 r1 r2.
  inductions E0; unfold simpl_fill in *;  intros.
  - inductions E; unfold simpl_fill in *; inverts* eq.
    inverts wf1.
    forwards*: step_not_walue red1.
  - inductions E; unfold simpl_fill in *; inverts* eq.
    inverts wf2.
    forwards*: step_not_walue red2.
  - inductions E; unfold simpl_fill in *; inverts* eq.
    inverts wf1.
    forwards*: step_not_value red1.
  - inductions E; unfold simpl_fill in *; inverts* eq.
    inverts wf2.
    forwards*: step_not_value red2.
Qed.


Lemma TypedReduce_prv_value2: forall v A v' l b,
    value v -> TypedReduce v l b A (Expr v') -> value v'.
Proof.
  introv Val Red.
  inductions Red; eauto.
  - inverts* Val.
  - inverts* Val; try solve[inverts H].
Qed.


Lemma principle_inf: forall v A,
ssval v -> Typing nil v Inf A -> (principal_type v) = A .
Proof.
  introv Val Typ.
  gen A.
  induction Val; introv  Typ; try solve [inverts* Typ].
Qed. 

Lemma sim_decidable:forall A B,
sim A B \/ ~ (sim A B).
Proof.
  introv.
  gen A.
  inductions B; intros; eauto.
  - inductions A; eauto. right. unfold not;  intros. inverts* H.
  - inductions A; eauto. right. unfold not; intros.
    inverts* H.
    forwards*: IHB1 A1. forwards*: IHB2 A2.
    destruct H.  destruct H0. left.  eauto. right.
    unfold not; intros. inverts* H1. destruct H0.
    right. unfold not; intros. inverts* H1.
    right. unfold not; intros. inverts* H1.
Qed.


Lemma TypedReduce_preservation2: forall v A v' B l b,
    value v -> TypedReduce v l b A (Expr v') -> Typing nil v Chk B -> Typing nil v' Inf A.
Proof with auto.
  introv Val Red Typ'.
  gen B.
  lets Red': Red.
  inductions Red; introv Typ;
  lets Lc : Typing_regular_1 Typ;
  try solve [constructor*].
  + destruct(lambda_decidable e).
    *
      apply Typ_anno; eauto.
      inverts Typ. inverts H3. inverts H12; try solve[inverts H2].
      eapply Typ_sim; eauto.
      forwards*: principle_inf H3. 
      rewrite H8 in H1; auto.
      forwards*: principle_inf H14. 
      rewrite H3 in H1; auto.
      eapply Typ_sim; eauto.
    *
      inverts* Val; try solve[exfalso; apply H2; auto].
  +
    inverts H.
    inverts Typ. inverts* H. 
    inverts* H9. inverts* H.
    destruct(sim_decidable (t_arrow A0 B0) (t_arrow C D)).
    eapply Typ_anno; eauto.
    eapply Typ_save; eauto. 
    inverts H13.
    inverts H11.
    destruct(sim_decidable (t_arrow A0 B0) (t_arrow C D)).
    eapply Typ_anno; eauto.
    eapply Typ_save; eauto. 
    inverts H14.
Qed.


Lemma value_nlam: forall v,
 value v ->
 nlam v.
Proof.
  introv val.
  inductions val; eauto.
Qed.

Lemma  TypeLists_unique: forall v r1 r2 A B,
    walue v -> Typing nil v Chk B ->  TypeLists v A r1 ->  TypeLists v A r2 -> r1 = r2.
Proof.
  introv Val H R1 R2.
  gen r2 B.
  lets R1': R1.
  induction R1; introv R2 typ;
    try solve [inverts* R2].
  - inverts* typ; try solve[inverts H0].
    inverts Val.
    forwards*: value_nlam H4.
    inverts* R2.
    forwards*: TypedReduce_unique H0 H12.
    forwards*: TypedReduce_unique H0 H12. inverts H14.
    forwards*: TypedReduce_unique H0 H13.
    inverts* H3.
  - inverts Val.
    forwards*: value_nlam H1.
    inverts typ; try solve[inverts H2].
    inverts* R2.
    forwards*: TypedReduce_unique H0 H12. inverts H14.
    inverts* H0.
  - inverts Val.
    forwards*: value_nlam H1.
    inverts typ; try solve[inverts H1].
    inverts* R2. inverts R1. inverts R1.
    forwards*: TypedReduce_unique H0 H13. inverts H6.
    forwards*: TypedReduce_prv_value2 H0.   
    forwards*: TypedReduce_preservation2 H0.
    forwards*: Typing_chk H7. inverts H8.
    forwards*: IHR1 R1 H14. inverts H5.
    forwards*: TypedReduce_unique H0 H13. inverts H6.
    inverts* H0.
  - inverts* R2. inverts* H7.
    inverts Val; try solve[inverts H0].
    inverts* H; inverts* H12; try solve[inverts H11]; try solve[inverts H10].
    inverts H7.
  - inverts Val.
    forwards*: value_nlam H1.
    inverts typ; try solve[inverts H0].
    inverts* R2.
    forwards*: TypedReduce_unique H0 H13. 
    forwards*: TypedReduce_unique H0 H13. inverts H6.
    inverts H5.
    inverts H0. 
Qed.


Theorem step_unique: forall A e r1 r2,
    Typing nil e Chk A -> step e r1 -> step e r2 -> r1 = r2.
Proof.
  introv Typ Red1.
  gen A r2.
  lets Red1' : Red1.
  inductions Red1;
    introv Typ Red2.
    - (* \x.e:a->b ---> \x.e:a->b:a->b *)
    inverts* Red2. destruct E; unfold simpl_fill in H0; inverts* H0.
    destruct E; unfold simpl_fill in H0; inverts* H0.
    inverts* H5. destruct E; unfold simpl_fill in H0; inverts* H0.
    inverts* H5. destruct E; unfold simpl_fill in H0; inverts* H0.
    inverts* H5. 
  - (* i ---> i:int *)
    inverts* Red2.  
    destruct E; unfold simpl_fill in H; inverts* H.
    destruct E; unfold simpl_fill in H; inverts* H.
  - (* e1 e2 ---> e1' e2 *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0];
    try solve[destruct E; unfold simpl_fill in H1; inverts* H1].
    + forwards*: sfill_eq H0.             
      destruct H3. subst. inverts Typ; 
      try solve[destruct E0; unfold simpl_fill in H3; inverts H3].
      destruct E0; unfold simpl_fill in H3; inverts H3. 
      forwards*: Typing_chk H12. inverts H3.
      forwards*: IHRed1 Red1 H2. congruence. 
      forwards*: IHRed1 Red1 H2. congruence.
      forwards*: IHRed1 Red1 H14. congruence. 
      forwards*: IHRed1 Red1 H15. congruence.
    + forwards*: sfill_eq H0.
      destruct H3. subst. inverts Typ;
      try solve[destruct E0; unfold simpl_fill in H3; inverts H3].
      destruct E0; unfold simpl_fill in H3; inverts H3. 
      forwards*: Typing_chk H12. inverts H3.
      forwards*: IHRed1 Red1 H2. inverts H3.
      forwards*: IHRed1 Red1 H2. inverts H3.
      forwards*: IHRed1 Red1 H14. congruence. 
      forwards*: IHRed1 Red1 H15. congruence.
    + destruct E; unfold simpl_fill in H0; inverts* H0.
      forwards*: step_not_walue Red1.
      forwards*: step_not_value Red1.
    + destruct E; unfold simpl_fill in H0; inverts* H0. 
      forwards*: step_not_value Red1.
      forwards*: step_not_walue Red1.
    + destruct E; unfold simpl_fill in H0; inverts* H0. 
      forwards*: step_not_value Red1.
      forwards*: step_not_walue Red1.
    + destruct E; unfold simpl_fill in H0; inverts* H0. 
      forwards*: step_not_value Red1.
      forwards*: step_not_walue Red1.
    + destruct E; unfold simpl_fill in H0; inverts* H0.
      forwards*: step_not_walue Red1.
      forwards*: step_not_walue Red1.
    + destruct E; unfold simpl_fill in H1; inverts* H1.
      forwards*: step_not_value Red1.
      forwards*: step_not_value Red1.
  - (* e1 e2 ---> blame e2 *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0];
    try solve[destruct E; unfold simpl_fill in H1; inverts* H1];
    try solve[destruct E; unfold simpl_fill in H1; inverts* H1;
    forwards*: step_not_value Red1];
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0;
    forwards*: step_not_value Red1;
    forwards*: step_not_value Red1];
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0;
    forwards*: step_not_value Red1;
    forwards*: step_not_walue Red1].
    + forwards*: sfill_eq H0.
     destruct H3. subst. inverts Typ; 
      try solve[destruct E0; unfold simpl_fill in H3; inverts H3].
      destruct E0; unfold simpl_fill in H3; inverts H3.
      forwards*: Typing_chk H12. inverts H3. 
      forwards*: IHRed1 Red1 H2. inverts H3.
      forwards*: IHRed1 Red1 H2. inverts H3.
      forwards*: IHRed1 Red1 H14. inverts H3.
      forwards*: IHRed1 Red1 H15. inverts H3.
    + forwards*: sfill_eq H0.
     destruct H3. subst. inverts Typ; 
      try solve[destruct E0; unfold simpl_fill in H3; inverts H3].
      destruct E0; unfold simpl_fill in H3; inverts H3.
      forwards*: Typing_chk H12. inverts H3. 
      forwards*: IHRed1 Red1 H2. 
      forwards*: IHRed1 Red1 H2. 
      forwards*: IHRed1 Red1 H14. 
      forwards*: IHRed1 Red1 H15. 
  - (* e:a ---> e':a *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0];
    try solve [inverts Typ; inverts H0; forwards*: IHRed1 H11; inverts* H0];
    try solve[forwards*: step_not_walue Red1];
    try solve [inverts Typ; inverts H0; forwards*: IHRed1 H11; congruence].
    inverts Typ. inverts H0. forwards*: IHRed1 H5; congruence.
    inverts H11.
    exfalso; apply H6; eauto.
    inverts Typ. inverts H0. forwards*: IHRed1 H5; congruence.
    inverts H11.
    exfalso; apply H6; eauto.
  - (* e:a ---> blame:a *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H0; inverts* H0];
    try solve[forwards*: step_not_walue Red1];
    try solve [inverts Typ; inverts H0; forwards*: IHRed1 H5; congruence].
    inverts Typ. inverts H0. forwards*: IHRed1 H5; congruence.
    inverts H11.
    exfalso; apply H; eauto.
    inverts Typ. inverts H0.
    forwards*: IHRed1 H5; congruence.
    inverts H11.
    exfalso; apply H; eauto.
  - (* \x.e v *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H2; inverts* H2;
    forwards*: step_not_walue H4;
    forwards*: step_not_value H4].
    inverts Typ. inverts H2. inverts H14; try solve[inverts H0].
    inverts H15.
    forwards*: TypedReduce_unique H1 H9. congruence.
  - (* \x.e:a1->b1:a2->b2 v ---> e[x->v']:b1:b2 *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H2; inverts* H2; simpl in *;
    forwards*: step_not_walue H4;
    forwards*: step_not_walue H4];
    try solve[inverts H0];
    inverts Typ. inverts H2. inverts H12; try solve[inverts H0].
    forwards*: TypeLists_unique H1 H17. congruence.
    forwards*: TypeLists_unique H1 H17. congruence.
    inverts H2. inverts H12; try solve[inverts H0].
    forwards*: TypeLists_unique H1 H17. congruence.
    forwards*: TypeLists_unique H1 H17. congruence.
  - (* \x.e:a1->b1:a2->b2 v ---> blame *)
     inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H2; inverts* H2; simpl in *;
    forwards*: step_not_walue H4;
    forwards*: step_not_walue H4];
    try solve[inverts H0];
    try solve [inverts Typ; inverts H2; inverts H12; try solve[inverts H0];
    forwards*: TypeLists_unique H1 H17; congruence]. 
  - (* v: a *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H1; inverts* H1];
    try solve[inverts H];
    try solve[forwards*: step_not_value H6].    
    inverts* Typ. inverts* H1. inverts* H12; try solve[inverts H].
    forwards*:  TypedReduce_unique H0 H7.
    forwards*:  TypedReduce_unique H0 H7.
  - (* v1+v2-> blame+ v2*)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H2; inverts* H2;
    forwards*: step_not_value H4; forwards*: step_not_value H4].
    inverts* H1.
    exfalso. apply H11; eauto.
    inverts* H1.
    exfalso. apply H9; eauto.
  - (* i+v2-> i+ blame*)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H1; inverts* H1;
    forwards*: step_not_value H3; forwards*: step_not_value H3].
     inverts* H0.
    exfalso; apply H1; eauto.
    inverts* H10.
    exfalso. apply H7; eauto.
    inverts* H0.
    exfalso. apply H8; eauto.
  - (* i:a + i: b *)
    inverts* Red2;
    try solve[destruct E; unfold simpl_fill in H; inverts* H;
    forwards*: step_not_value H1; forwards*: step_not_value H1].
    inverts* H8.
    exfalso. apply H9; eauto.
    inverts* H10.
    exfalso. apply H6; eauto.
Qed.



