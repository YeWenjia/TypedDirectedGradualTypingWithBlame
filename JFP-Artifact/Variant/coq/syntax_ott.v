...
Inductive TypedReduce : exp -> typ -> res -> Prop :=    (* defn TypedReduce *)
 ...
 | TReduce_save : forall (s:exp) (A B C D:typ),
      sval s ->
     TypedReduce (e_anno s A)  (t_arrow C D)  (Expr (e_anno s (t_arrow C D)))
 | TReduce_p : forall (e:exp) (A B C:typ),
     TypedReduce  (e_anno (e_anno (e_abs e) (t_arrow A B)) C)  t_int Blame.
...
Inductive TypeLists : exp -> list typ -> res -> Prop :=    (* defn TypeLists *)
 ...
 | TLists_abs : forall B t1 t2 e,
     not(B = t_int) ->
     TypeLists  (e_abs e) (cons (t_arrow t1 t2) (cons B nil)) (Expr (e_anno (e_anno (e_abs e) (t_arrow t1 t2)) B))
 | TLists_absi : forall t1 t2 e,
     TypeLists  (e_abs e) (cons (t_arrow t1 t2) (cons t_int nil)) Blame
...
(** infrastructure *)
Hint Constructors ssval value nlam walue gvalue Ground gvalue gwalue sval vvalue ival sim TypedReduce gTypedReduce TypeLists gTypeLists wellformed simpl_wf step gstep steps gsteps tpre epre Typing gtyping lc_exp : core.
