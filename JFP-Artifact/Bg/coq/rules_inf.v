...
Fixpoint close_exp_wrt_exp_rec (n1 : nat) (x1 : var) (e1 : exp) {struct e1} : exp :=
  match e1 with
    ...
    | e_lit i1 => e_lit i1
    | e_abs t1 e2 t2 => e_abs t1 (close_exp_wrt_exp_rec (S n1) x1 e2) t2
...
Fixpoint size_exp (e1 : exp) {struct e1} : nat :=
  match e1 with
    ...
    | e_var_b n1 => 1
    | e_lit i1 => 1
    | e_abs t1 e2 t2 => 1 + (size_typ t1) + (size_exp e2) + (size_typ t2)
...
Inductive degree_exp_wrt_exp : nat -> exp -> Prop :=
  ...
  | degree_wrt_exp_e_abs : forall n1 e1 t1 t2,
    degree_exp_wrt_exp (S n1) e1 ->
    degree_exp_wrt_exp n1 (e_abs t1 e1 t2)
...
forall x2 e2 e1 x1 t1 t2,
  lc_exp e1 ->
  x2 `notin` fv_exp e1 `union` fv_exp e2 `union` singleton x1 ->
  subst_exp e1 x1 (e_abs t1 e2 t2) = e_abs t1 (close_exp_wrt_exp x2 (subst_exp e1 x1 (open_exp_wrt_exp e2 (e_var_f x2)))) t2.
Proof.
default_simp.
Qed.
