...
Fixpoint close_exp_wrt_exp_rec (n1 : nat) (x1 : var) (e1 : exp) {struct e1} : exp :=
  match e1 with
    ...
    | e_lit i1 => e_lit i1
    | e_abs t1 e2 l b t2 => e_abs t1 (close_exp_wrt_exp_rec (S n1) x1 e2) l b t2
    ...
    | e_add => e_add
    | e_addl i1 => e_addl i1
    | e_appv e2  e3 => e_appv (close_exp_wrt_exp_rec n1 x1 e2) (close_exp_wrt_exp_rec n1 x1 e3)
  end.
...
Fixpoint size_exp (e1 : exp) {struct e1} : nat :=
  match e1 with
    ...
    | e_var_b n1 => 1
    | e_lit i1 => 1
    | e_abs t1 e2 l b t2 => 1 + (size_typ t1) + (size_exp e2) + (size_typ t2)
    | e_app e2 p b e3 => 1 + (size_exp e2) + (size_exp e3)
    | e_anno e2 p b A1 => 1 + (size_exp e2) + (size_typ A1)
    | e_add => 1
    | e_addl i1 => 1
    | e_appv e2 e3 => 1 + (size_exp e2) + (size_exp e3)
    (* | e_absr e2 l b => 1 +  (size_exp e2)  *)
  end.
...
Inductive degree_exp_wrt_exp : nat -> exp -> Prop :=
  ...
  | degree_wrt_exp_e_abs : forall n1 e1 l b t1 t2,
    degree_exp_wrt_exp (S n1) e1 ->
    degree_exp_wrt_exp n1 (e_abs t1 e1 l b t2)
  | degree_wrt_exp_e_app : forall n1 e1 e2 p b ,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 e2 ->
    degree_exp_wrt_exp n1 (e_app e1 p b e2)
  | degree_wrt_exp_e_anno : forall n1 e1 A1 p b ,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (e_anno e1 p b A1)
    | degree_wrt_exp_e_add : forall n1,
    degree_exp_wrt_exp n1 (e_add)
  | degree_wrt_exp_e_addl : forall n1 i1,
    degree_exp_wrt_exp n1 (e_addl i1)
  | degree_wrt_exp_e_appv : forall n1 e1 e2 ,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 e2 ->
    degree_exp_wrt_exp n1 (e_appv e1 e2).
  (* | degree_wrt_exp_e_absr : forall n1 e1 l b,
    degree_exp_wrt_exp (S n1) e1 ->
    degree_exp_wrt_exp n1 (e_absr e1 l b). *)
...
Inductive lc_set_exp : exp -> Set :=
  ...
  | lc_set_e_lit : forall i1,
    lc_set_exp (e_lit i1)
  | lc_set_e_abs : forall e1 l b t1 t2,
    (forall x1 : var, lc_set_exp (open_exp_wrt_exp e1 (e_var_f x1))) ->
    lc_set_exp (e_abs t1 e1 l b t2)
  | lc_set_e_app : forall e1 e2 p b,
    lc_set_exp e1 ->
    lc_set_exp e2 ->
    lc_set_exp (e_app e1 p b e2)
  | lc_set_e_anno : forall e1 p b A1,
    lc_set_exp e1 ->
    lc_set_exp (e_anno e1 p b A1)
    | lc_set_e_add :
    lc_set_exp (e_add)
  | lc_set_e_addl : forall i1,
    lc_set_exp (e_addl i1)
  | lc_set_e_appv : forall e1 e2,
    lc_set_exp e1 ->
    lc_set_exp e2 ->
    lc_set_exp (e_appv e1 e2).
  (* | lc_set_e_absr : forall e1 l b ,
    (forall x1 : var, lc_set_exp (open_exp_wrt_exp e1 (e_var_f x1))) ->
    lc_set_exp (e_absr e1 l b). *)
...
forall x1 e1 l b t1 t2,
  lc_exp (open_exp_wrt_exp e1 (e_var_f x1)) ->
  lc_exp (e_abs t1 e1 l b t2).
Proof.
intros; exp_lc_exists_tac; eauto with lngen.
Qed.

Hint Extern 1 (lc_exp (e_abs _ _ _ _ _)) =>
  let x1 := fresh in
  pick_fresh x1;
  apply (lc_e_abs_exists x1) : core.

(* 
  Lemma lc_e_absr_exists :
  forall x1 e1 l b,
    lc_exp (open_exp_wrt_exp e1 (e_var_f x1)) ->
    lc_exp (e_absr e1 l b).
  Proof.
  intros; exp_lc_exists_tac; eauto with lngen.
  Qed.
  
  Hint Extern 1 (lc_exp (e_absr _ _ _)) =>
    let x1 := fresh in
    pick_fresh x1;
    apply (lc_e_absr_exists x1) : core. *)
...
forall x2 e2 e1 x1 l b t1 t2,
  lc_exp e1 ->
  x2 `notin` fv_exp e1 `union` fv_exp e2 `union` singleton x1 ->
  subst_exp e1 x1 (e_abs t1 e2 l b t2) = e_abs t1 (close_exp_wrt_exp x2 (subst_exp e1 x1 (open_exp_wrt_exp e2 (e_var_f x2)))) l b t2.
Proof.
default_simp.
Qed.
