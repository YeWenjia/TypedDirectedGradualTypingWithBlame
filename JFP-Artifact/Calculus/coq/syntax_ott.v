...
Inductive ttyping : ctx -> exp -> dirflag -> typ -> term -> Prop :=    (* defn Typing *)
 ...
 (* | ttyp_absd : forall (L:vars) (G:ctx) (e:exp) (t:term),
     ...
     ttyping G (e_abs e) Chk t_dyn (trm_abs A t) *)
...
Inductive step : exp -> res -> Prop :=    (* defn step *)
  ...
  | Step_nbeta : forall (e:exp) (v v' : exp),
    lc_exp (e_abs e) ->
    value v ->
    TypedReduce v t_dyn (e_exp v') ->
    step (e_app (e_abs e) v)  (e_exp (e_anno (open_exp_wrt_exp  e v' ) t_dyn) ) 
 | Step_beta : forall (A:typ) (e:exp) (B:typ) (v v' : exp),
    lc_exp (e_abs e) ->
    value v ->
    TypedReduce v A (e_exp v') ->
    step (e_app (e_anno (e_abs e) (t_arrow A B)) v)  (e_exp (e_anno (open_exp_wrt_exp  e v' ) B) )  
 | Step_addL : forall (A:typ) (e:exp) (B:typ) (v v' : exp),
    value v ->
    TypedReduce v A (e_exp v') ->
    step (e_app (e_anno (e_add) (t_arrow A B)) v)  (e_exp (e_anno (e_app e_add v') B) )  
 | Step_addR : forall (A:typ) (e:exp) (B:typ) (v v' : exp) i,
    value v ->
    TypedReduce v A (e_exp v') ->
 ...
 | Step_abeta : forall (v1:exp) (A B C D:typ) (v2 v2':exp),
     value (e_anno v1 (t_arrow A B))  ->
     TypedReduce v2 C (e_exp v2') ->
     value v2 ->
     step (e_app (e_anno (e_anno v1 (t_arrow A B)) (t_arrow C D))  v2) (e_exp (e_anno (e_app (e_anno v1 (t_arrow A B)) v2') D)).
...
(** infrastructure *)
Hint Constructors Ground walue value sim TypedReduce step steps wellformed Typing ttyping lc_exp : core.
