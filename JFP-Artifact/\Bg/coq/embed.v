...
Fixpoint embed (e:nexp) : exp :=
  ...
  | (ne_abs e) => (e_anno (e_abs (embed(e))) t_dyn)
  end.
...
Proof.
  ...
  inverts* s.
  -
  assert(embed (ne_abs e1) = (e_anno (e_abs (embed e1)) t_dyn)).
  unfold embed; eauto.
  rewrite H.
  simpl; eauto.
  simpl in *.
...
Proof.
    ...
    -
      ...
      rewrite <- embed_open.
      destruct(lambda_decidable (embed (open_nexp_wrt_nexp_rec 0 (ne_var_f x) e))).
      inverts H2.
      rewrite H3 in *. inverts H1.
      eapply Typ_sim; eauto.
    -
    eapply Typ_app; eauto.
    destruct(lambda_decidable (embed e2)).
    inverts H.
    rewrite H0 in *. inverts IHwl2.
    eapply Typ_sim; eauto.
Qed.
