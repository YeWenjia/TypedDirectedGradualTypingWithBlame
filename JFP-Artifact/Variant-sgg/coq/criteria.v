...
Proof.
  ...
  - inverts* ep. 
    ...
    inverts H2. inverts H0. inverts* H7. inverts H0. inverts H2.
    inverts H12.
    exists* (e_anno (e_anno (e_abs e0) (t_arrow t_dyn t_dyn)) (t_arrow B0_1 B0_2)).
    splits*.
    eapply star_trans.
    apply star_one.
   ...
   inverts Typ.
    destruct B0; try solve[inverts* H2; inverts* H0].
    inverts H2. inverts H0. inverts* H9. inverts H0. inverts H2.
    inverts H14.
    exists*.
    exists*.
Qed.
...
Proof.
  ...
  -
    ...
    +
    inverts ep. inverts* H7;try solve[inverts* val2].
    inverts* H9. inverts* H6;try solve[inverts* val2].
    inverts val2.
    simpl in *.
  ...
  -
   inverts val1. inverts ep.
   inverts* H4;try solve[inverts* val2].
   inverts* H6. inverts* H3;try solve[inverts* val2].
   inverts val2. 
   inverts* tp.
   exists*.
   exists* (e_anno (e_anno (e_abs e0) (t_arrow C0 D0)) (t_arrow C1 D1)).
   splits*.
   apply ep_anno;eauto.
Qed.
...
Lemma typlist_guarantee2: forall e1 e2 e2' e1' A B A1 A2 A3 A4 B1 B2 B3 B4 v',
 value e2' ->
 value e2 ->
 lc_exp (e_abs e1) ->
 Typing nil e2 Chk A ->
  ...
  - inverts red.
    + destruct E; unfold simpl_fill in H; inverts* H.
      *
      ...
      forwards*: IHep1 H4. inverts H. inverts H10.
      ...
      *
      ...
      forwards*: Typing_chk H9. inverts H4.
      forwards*: val_prel ep1. 
      inverts H4. inverts H15. inverts H16.
      forwards*: Typing_regular_1 H12.
      forwards*: epre_lc ep2.
      exists(e_app x1 x). split. 
      apply star_trans with (b:=e_app x1 e2').
      apply multi_red_app2;auto.
      apply multi_red_app;auto.
      unfold simpl_fill. eauto.
      forwards*: epre_lc ep1.
      inverts ep1.
      exists(e_app (e_abs e0) x). splits.
      apply multi_red_app;auto.
      unfold simpl_fill. eauto.
    + 
      forwards*: epre_lc ep2.
      ...
      inverts H4.
      *
      inverts typ1. inverts H4. forwards: Typing_chk H9. inverts H4.
      forwards*: val_prel ep1. inverts H4. inverts H8. inverts H10.
      inverts H0;inverts H3; simpl in *;
      try solve[inverts H0; inverts H9; try solve[inverts H16]; 
      try solve[inverts H10; inverts H0; inverts H3]].
      subst.
      inverts H13. inverts H16. inverts H17.
      inverts H13; try solve[inverts H8].
      inverts* H14.
      --
      forwards*: multi_red_app2 H H4.
      exists (
       e_app (e_anno (e_anno (e_abs e3) (t_arrow C D)) 
       (t_arrow t_dyn t_dyn)) (e_anno (e_abs e2) t_dyn)).
      ...
      --
      forwards*: multi_red_app2 H H4. 
      exists((e_app (e_anno (e_anno (e_abs e3) (t_arrow C D)) (t_arrow C0 D0))
      (e_anno (e_abs e2) C0))).
      splits.
      eapply star_trans.
      apply H0.
      ...
      *
      ...
      exists((e_app (e_abs e3) (e_anno (e_abs e2) t_dyn))).
      splits*.
    + 
      inverts typ1. inverts typ2.  inverts H. inverts H5.
      inverts H10. inverts H12. 
      forwards*: val_prel ep2. inverts H. inverts H5. inverts H10.
      forwards*: epre_lc ep1. 
      inverts ep1.
      inverts H9.
      forwards*: multi_red_app (e_abs e0) H.
      forwards*: preservation_multi_step H.
      forwards*: tdynamic_guarantee H11 H4. inverts H17. inverts H19.
      exists (e_anno (e0 ^^ x0) t_dyn). split*.
      pick fresh xx.
    ...
    +
      ...
      forwards*: val_prel ep1. 
      inverts H8. inverts H11. inverts H13.
      forwards*: val_prel ep2. 
      inverts H13. inverts H17. inverts H18.
      inverts H16. inverts H23. inverts H24.
      inverts H20; try solve[inverts* H11].
      forwards*: TypeLists_prv_value H4.
      forwards*: preservation_multi_step H13.
      inverts H21.
      *
      assert(tpre (t_arrow A1 B1) (t_arrow C D));auto.
      assert(tpre (t_arrow A2 B2) (t_arrow t_dyn t_dyn));auto.
      assert(e_app (e_anno (e_anno (e_abs e0) (t_arrow C D)) t_dyn) x0 ->*
      (Expr (e_app (e_anno (e_anno (e_abs e0) (t_arrow C D)) (t_arrow t_dyn t_dyn)) x0))).
      ...
      apply do_step;eauto.
      forwards*: typlist_guarantee2 H21 H22 H19 H4.
      inverts H26. inverts* H27.
      forwards*: multi_red_app2 H H8.
      forwards*: multi_red_app (e_anno (e_anno (e_abs e0) (t_arrow C D)) t_dyn) H13.
      exists x. splits*.
      *
      assert(tpre (t_arrow A1 B1) (t_arrow C D));auto.
      assert(tpre (t_arrow A2 B2) (t_arrow C0 D0));auto.
      assert(A5=(t_arrow A2 B2)). inverts* H12.
      subst. inverts H14.
      forwards*: typlist_guarantee2 H21 H22 H19 H4.
      inverts H14. inverts H24.
      forwards*: multi_red_app2 H H8.
      forwards*: multi_red_app (e_anno (e_anno (e_abs e0) (t_arrow C D)) (t_arrow C0 D0)) H13.
    +
      forwards*: value_lc H3.
      forwards*: epre_lc ep1.
      inverts typ1. inverts H1.
      forwards*: Typing_chk H8. inverts H1.
      forwards*: val_prel ep1.
      inverts H1. inverts H7. inverts H9.
      inverts H12. inverts* H14.
      forwards*: Typing_regular_1 H11.
      forwards*: epre_lc ep2.
      forwards*: multi_red_app2 H12 H1.
      inverts H2.
      forwards*: val_prel ep2. inverts H2. inverts H15. inverts H17.
      assert(walue (e_anno e0 t_dyn));auto.
      forwards*: multi_red_app H17 H2.
      exists((e_app (e_anno (e_anno e0 t_dyn) (t_arrow t_dyn t_dyn)) x0)).
      splits*.
      eapply star_trans;eauto.
      inverts ep2.
      exists((e_app (e_anno (e_anno e0 t_dyn) (t_arrow t_dyn t_dyn)) (e_abs e3))).
      splits*.
      eapply ep_app;eauto.
  - inverts red.
    + destruct E; unfold simpl_fill in H; inverts* H.
      ...
      inverts H1.
      forwards*: val_prel ep1.
      inverts H1. inverts H4. inverts H13.
      forwards*: Typing_regular_1 H12.
      forwards*: multi_red_add2 H13 H1.
      forwards*: multi_red_add H4 H.
      exists((e_add x0 x)). split*. 
      unfold simpl_fill. eauto.
    + 
  ...
  -  inverts* red.
    ...
    + 
      ...
      unfold not; intros nt; inverts* nt; try solve[
        inverts ep; try solve[inverts* H2];
        try solve[inverts* H20;inverts* H19;inverts* H2]
      ].
      forwards*: preservation_multi_step H8.
      forwards*: multi_red_anno B H8.
      forwards*: tdynamic_guarantee H13 H H4. inverts H18. inverts H19.
      exists* (x2).
Qed.
